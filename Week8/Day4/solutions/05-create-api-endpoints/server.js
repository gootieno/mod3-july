  ...
  req.on("end", () => {
    ...
    if (
      ...
      if (urlParts.length === 3) {
        ...
        // Your code here
        const { name, age } = req.body;
        let myDog = dogs.find((dog) => dog.dogId == dogId);

        name ? (myDog.name = name) : null;
        age ? (myDog.age = age) : null;

        res.statusCode = 201;
        res.setHeader("Content-Type", "application/json");
        return res.end(JSON.stringify(myDog));
      }
    }
    ...
    if (req.method === "DELETE" && req.url.startsWith("/dogs/")) {
      ...
      if (urlParts.length === 3) {
        ...
        // let index = dogs.findIndex((dog) => dog.dogId == dogId);
